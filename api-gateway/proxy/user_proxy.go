...
func (p *UserProxy) GetUser(c *gin.Context) {
	userID := c.Param("id")
	p.proxyRequestWithoutHMAC(c, "/users/"+userID, nil)
}
...
func (p *UserProxy) ListAdmins(c *gin.Context) {
	p.proxyRequestWithoutHMAC(c, "/users/admin", nil)
}

// ListCustomers handles list customers requests
func (p *UserProxy) ListCustomers(c *gin.Context) {
	p.proxyRequestWithoutHMAC(c, "/users/customer", nil)
}
...
// proxyRequest proxies a request to the User service
func (p *UserProxy) proxyRequestWithoutHMAC(c *gin.Context, path string, transformRequestBody func([]byte) ([]byte, error)) {
	targetURL := p.baseURL + path
